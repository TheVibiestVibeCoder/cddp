            ...
            <div class="card divide-y divide-ink-100">
                ...
                <div class="px-4 py-3.5 flex items-center justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-ink-950">{{ $cat->name }}</p>
                        @if($cat->description)
                        <p class="text-xs text-ink-500 mt-0.5">{{ $cat->description }}</p>
                        @endif
                        @if($cat->children->isNotEmpty())
                        <div class="mt-2 space-y-1 pl-4 border-l-2 border-ink-100">
                            @foreach($cat->children as $child)
                            <div class="flex items-center justify-between gap-2">
                                <p class="text-xs text-ink-700">{{ $child->name }}</p>
